model_ngrams_kbo <- function(text_str, model_ngram_size, gamma_vec, ns_ngrams_counts) {
  ...
  #                   and so forth
  # gamma_vec: a vector of discounts that shift probability from 
  #            observed to unobserved n-grams at each n-gram level
  #            with length equal to model_ngram_size and this order:
  #            gamma_vec[1]: gamma_2: discount applied to observed bigram  counts
  #                          (always needed)
  #            gamma_vec[2]: gamma_3: discount applied to observed trigram counts 
  #                          (needed if model_ngram_size >= 3)
  #            gamma_vec[3]: gamma_4: discount applied to observed 4-gram  counts
  #                          (needed if model_ngram_size >= 4)
  #            gamma_vec[4]: gamma_5: discount applied to observed 5-gram  counts
  #                          (needed if model_ngram_size >= 5)
  #            and so forth
  ...
  #                   ns_ngrams_counts is optional and if missing, it will be loaded
  ...
  # The n-grams used to make the prediction model (ns_ngrams_counts) were obtained with 
  # make_ngrams_scr.R and make_ngrams_from_tokens.R.
  # Each string should contain only fully lowercase words and single interior spaces.
  # There should be no leading or trailing spaces, no punctuation 
  # (except apostrophe for possesives and contractions), and no numbers.
  ...
  if (missing(ns_ngrams_counts)) {
    
    # load corpora n-grams and counts used for model training, if not supplied as argument
    # specify directory containing results of make_ngrams_scr.R
    dir_wd <- "C:\\Users\\josep\\data_science\\Data_Science_Capstone"
    # load output from make_ngrams_scr.R for use in model
    ...
    fn_ngrams_counts <- paste0(dir_wd, "\\", "ns_ngrams_counts_for_model_", as.character(model_ngram_size), ".RData")
    load(file = fn_ngrams_counts)
    # stack ns_ngrams_counts_*_tr into one argument that can be passed to model_ngrams_kbo,
    # instead of trying to handle a variable number of arguments in model_ngrams_kbo, which is used recursively
  ...
  # n-grams and counts at the main n-gram level
  eval(parse(text = paste0("ns_ngrams_counts_mn <- filter(ns_ngrams_counts, n == ", as.character(model_ngram_size)  , ")")))
  ngrams_counts_mn <- select(ns_ngrams_counts_mn, ngram, ngram_count)
  ...
  ngrams_counts_bo <- select(ns_ngrams_counts_bo, ngram, ngram_count)
  rm(ns_ngrams_counts_bo)
  # unigrams and counts
  ns_ngrams_counts_1 <- filter(ns_ngrams_counts, n == 1)
  ngrams_counts_1 <- select(ns_ngrams_counts_1, ngram, ngram_count)
  ...
  # extract gamma that will be applied at this n-gram level
  gamma_mn <- gamma_vec[model_ngram_size-1]
  # retain remaining gammas for recursive function call
  if (model_ngram_size > 2){
    gamma_bo_vec <- gamma_vec[1:(model_ngram_size-2)]
  }
  rm(gamma_vec)
  ...
  # counts
  counts_mn_pf_mn_o <- ngrams_counts_mn_pf_mn_o$ngram_count
  # discounted counts
  counts_mn_pf_mn_o_star <- counts_mn_pf_mn_o - gamma_mn
  # probabilities
  ...
  if (model_ngram_size == 2) {
    ...
    ngrams_probs_bo <- model_ngrams_kbo(pf_bo, model_ngram_size-1, gamma_bo_vec, ns_ngrams_counts)
